...
- **Rollback is deployment-level**: `rpm-ostree rollback` reverts the whole system to the previous deployment, and `rpm-ostree pin` keeps a deployment from being garbage collected ([scripts/linux/rollback-deploy.sh](scripts/linux/rollback-deploy.sh) reports the deployment list and whether a rollback candidate exists); for system-level damage, reinstall in place (see [docs/07-rescue.md](docs/07-rescue.md)) — the data disk `D:` is untouched either way.
