...
- Windows 11 Pro on the Windows side, **Kubuntu 26.04 LTS** (Plasma 6.6, Wayland-only, Calamares installer, three-year LTS window, kernel 7.0) on the Linux side
...
- **The LTS window is three years** with security updates only, so a release upgrade (`do-release-upgrade`) is a rare, planned event with its own card rather than a recurring chore.
