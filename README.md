...
A reproducible, reversible playbook for Windows 11 Pro + Fedora 44 Silverblue dual-boot on identical fresh devices.

This repository is a deployment playbook, not an installer. It ships the step manuals (three tracks, L0 to L5), the artifact contract each stage must leave behind, one script per step card (Windows-side PowerShell and Silverblue-side shell) with their shared contract libraries, and the partition, verification and risk registers that pin everything down. Every card names the script that judges it, and every script defaults to the safe direction: `--check` / `-Check` prints a verdict and writes nothing, and only an explicit `--apply` / `-Apply` (usually with `--yes` / `-Yes`) touches the system.

The material has two layers. The design documents ([docs/design/00-design.md](docs/design/00-design.md) plus its variant, card-format and step-automation companions) record *why* the design looks the way it does: target device class, four invariants, key decisions with the rejected alternatives, the recovery matrix and the 34-entry risk register. The manuals ([docs/00-overview.md](docs/00-overview.md) onwards) are the executable part: cards with a 做 / 看到 pass criterion and a 出错时 pointer.
...
- [Atomic semantics and rollback](#atomic-semantics-and-rollback)
...
This playbook assumes a clean-slate install instead, so the partition table is decided once rather than edited surgically later, and it treats "remove Linux safely" as a first-class procedure rather than an afterthought. The Linux side is an atomic OS, so the same premise pays off twice: rollback becomes a deployment switch rather than a filesystem-snapshot stack, and a failed upgrade never has to be repaired in place. Both premises, and the full cause analysis, are in [docs/00-overview.md](docs/00-overview.md) and sections 1, 3 and 4 of the design document.
...
Every step card also names its script, and the card ↔ script binding is machine-checked in both directions (`scripts/repo/check-docs.sh` rules C9b/C9c/C9d against `scripts/*/steps.tsv`). Scripts are read-only by default as described above; the silverblue-side ones additionally require root and `--yes` for any write.
...
| **L** | Silverblue only | about 6 | install, first boot (driver/mount/time/layering), converge, rollback drill |
...
scripts/linux/         per-card Silverblue-side scripts plus shared contract libraries
scripts/repo/          self-checks for this repository (docs structure, script syntax)
templates/             diskpart script and fstab/GRUB/rpm-ostreed/journald/user-dirs snippets
...
|---|---|---|
| I1 | `BootOrder` always lists **Windows Boot Manager first** | After the Linux partitions are deleted, the firmware still points at the now-missing `\EFI\fedora\grubx64.efi` and the machine stops at `grub rescue>` |
...
- Windows 11 Pro on the Windows side, **Fedora 44 Silverblue** (atomic, GNOME 50, Wayland session) on the Linux side

Documented deviations get either an adaptation branch (two or more disks, a disk far from 1 TB, discrete-GPU-only, a non-GNOME desktop via Kinoite, a read-only shared mount, a shared-ESP fallback) or are explicitly **not supported** by v1: disk encryption, a locked VMD/RAID controller mode, or a firmware that can only boot from the first disk. The device parameter table that absorbs vendor differences (`DISK`, `VENDOR`, `BOOT_MENU_KEY`, `DISK_MODEL`, `DISK_SIZE`, `UBLUE_IMAGE`, ...) is defined in [docs/00-overview.md](docs/00-overview.md) and is filled in once per device. The ublue NVIDIA image and branch names are marked as **to be verified** against upstream documentation; `UBLUE_IMAGE` must not be treated as a fixed value.
...
| 6 | `/boot` | 1 GiB | ext4 | required to be separate on an atomic OS; every deployment's kernel and initrd live here |
| 7 | Fedora root | approx. 113 GiB | btrfs | `/`, ostree deployments plus the `var` subvolume (`/home` is a symlink to `/var/home`) |
...
- **The three Fedora partitions are created later, inside the 115 GiB unallocated region reserved in L1.** The L1 `diskpart` script stops at `D:` and deliberately leaves the rest unallocated; the L3 installer cuts `ESP-Fedora` 1 GiB + `/boot` 1 GiB + root approx. 113 GiB out of that region.
...
## Atomic semantics and rollback

Fedora Silverblue is an atomic OS, and the playbook leans on that instead of fighting it:
...
- **Deliberately not used**: `snapd` (applications are Flatpak, the system layer is `rpm-ostree`) and `snapper` / `grub-btrfs` / btrfs snapshot rollback (rollback is deployment-level, not filesystem-snapshot-level; the design document lists this as a rejected alternative).
...
- **Silverblue**: an approx. 113 GiB root plus a separate 1 GiB `/boot`. Documents, downloads, pictures and desktop live on the shared volume, which is what keeps root small; and because `/var/home` is not part of a deployment, a bad upgrade is undone by a deployment rollback without touching user data.
...
- **Silverblue is broken** -> back up `/var/home` to the shared volume first, then reinstall Silverblue formatting the root partition (btrfs) only, keeping `/boot` and the Fedora ESP (mounted, never formatted), so the Windows side and the shared data survive.
...
1. **Pin the current deployment before any change** (`rpm-ostree pin` ahead of layering, rebase or a release upgrade);
2. **Deployment-level rollback**: boot menu or `rpm-ostree rollback`, with a post-rollback re-check of session and module state;
3. **Multiple deployments plus one-shot boot**: every deployment carries its own kernel and initrd in the separate `/boot`, and firmware-level `BootNext` complements deployment selection without violating I2;
4. **A permanent rescue medium**: the Fedora install USB doubles as a live environment and is not recycled;
5. **Crash observability**: journald made persistent, so `journalctl -b -1` still works after a failed boot (`/var` is not rolled back);
6. **OOM and memory pressure**: zram verified plus a 4 GiB swapfile, with `systemd-oomd` enabled;
7. **An always-on SSH rescue channel**, so a dead desktop can still be debugged from another machine (`/etc` is persistent, so it survives a rollback);
8. **A conservative update policy**: `rpm-ostreed-automatic` only checks and downloads — it never applies and never reboots;
...
- **B. system function (B1-B11)** — Wayland session, GPU driver healthy with nouveau as fallback, Secure Boot still on, `ntfs3` read-write mount with `nofail`, bidirectional visibility on the shared volume, ublue image source shown by `rpm-ostree status`, layered package list as planned, redirected home directories, RTC in UTC, Bluetooth pairing surviving a system switch, `fwupd` seeing the device.
...
- **F. robustness (F1-F9)** — a real deployment rollback drilled once (with user data still present afterwards), pinning usable, multiple deployments available, journald persistent, update policy as configured, SSH rescue reachable, `systemd-oomd` and zram active, `smartd` reporting PASSED, and `nofail` present on the entries L4 writes while deliberately absent from `/boot/efi`.
...
The register — risk, consequence and mitigation — lives in section 9 of [docs/design/00-design.md](docs/design/00-design.md) (34 entries), with a per-stage quick lookup plus 20 symptom cards in [docs/10-faq.md](docs/10-faq.md). It covers Intel VMD/RAID controller modes, BitLocker recovery prompts triggered by partition or firmware changes, Windows updates rewriting its ESP or the SBAT/DBX episode, NVIDIA module signatures on the atomic side, Fast Startup plus dual NTFS writers, firmware that only boots from the first disk, picking the wrong target disk, RTC and Bluetooth state divergence, `ntfs3` write damage on the shared volume, the upstream Anaconda failure on a disk that already holds a system, and hardware faults misdiagnosed as dual-boot problems.

The Windows activation route is also a risk entry: the playbook documents the flow and links to the upstream project rather than shipping any activation script, and the repository deliberately contains no such script. Install-media checks split by vendor reality: the Fedora ISO is compared against the official `CHECKSUM` file, while the Windows ISO is limited to "official download domain plus official installer verification", because Microsoft does not publish SHA256 values for Windows 11 ISOs ([docs/01-firmware.md](docs/01-firmware.md)).
