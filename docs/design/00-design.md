...
用户数据与浏览器凭据迁移 / 磁盘加密与 TPM-FDE / 休眠 / btrfs 快照回滚 / 自定义 Secure Boot 密钥 / 图形化安装器 / 多发行版模板。
...
| NTFS 内核驱动 `ntfs3` 支持读写,但无 POSIX 权限语义(挂载时以固定 uid/gid/umask 呈现) | 已确认;需在挂载选项与工作流上适配(见 5.3) |
...
| 4 | Windows 数据 D: | **≈590GiB** | NTFS | 游戏库、下载、文档、容器镜像;已知文件夹重定向的目标;**双系统共享分区(3.16 / 5.3)** |
...
**结论:有,就是 Windows 数据分区 `D:`(≈590GiB,NTFS)。** Windows 侧原生访问,Ubuntu 侧以 `ntfs3` **读写**挂载。办公文件集中放在约定目录(例如 `D:\Shared\`)下,切系统就能接着干,不需要拷贝或传输。
...
**B. 系统功能组**:会话类型为 `wayland`(且无 X11 会话可选);GPU 驱动状态正常或有 nouveau 兜底且无签名拒绝日志;Secure Boot 保持开启;共享数据分区以 `ntfs3` **读写**挂载成功且 `nofail`;**跨系统双向可见性**(Windows 写入 → Linux 读到;Linux 写入 → Windows 读到);`RTC in local TZ: no`;切换系统后蓝牙无需重新配对;`fwupd` 能识别设备。
...
| **Fast Startup + 双写 NTFS** | 共享数据分区损坏 | L1 强制关闭 Fast Startup 与休眠;共享盘禁止在 Windows 处于休眠状态时被 Linux 挂载 |
...
| **KMS 续期依赖可达的 KMS 主机** | 180 天周期内未成功续期会导致激活失效 | 保留续期任务并定期核对激活状态;失效时重新执行一次在线激活流程 |
| **`ntfs3` 写入导致共享盘数据损坏** | 办公文件丢失 | 关键目录保留第二份备份;避免在 Linux 侧批量重命名/移动大目录 |
| **共享盘被 BitLocker/设备加密** | Linux 侧无法挂载,共享方案失效 | 装机时 `D:` 保持不加密;若 Windows 自动启用设备加密,先解密再继续 |
| **POSIX 语义差异** | 依赖符号链接/权限位的工具在共享盘上报错 | 共享盘只放文档类数据;代码与工程目录留在 Linux 本地 root |
