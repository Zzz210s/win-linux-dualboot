...
- 整盘重排:ESP 2GiB → MSR 16MiB → C: ≈790GiB → **为 Ubuntu 预留 160GiB 未分配空间(root 140 + 快照 20)** → WinRE;
...
| 4 | Ubuntu root | **140GiB** | ext4 | `/`(内核位于 root 内的 `/boot`,不额外分区) |
| 5 | Snapshot | **20GiB** | ext4 | `/snapshots`,变更前快照的存放位置(健壮性 R1/R2) |
| 6 | WinRE | 1GiB | Recovery | Windows 恢复环境,置于磁盘末尾 |

Linux 侧合计 160GiB(= 140 + 20),与决策 3.6 一致。
