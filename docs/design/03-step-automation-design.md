...
**合计**:动作卡 **47 张**(01 四 + 02 四 + 03 九 + 04 四 + 05 十三 + 07 十三),对应**步骤脚本 45 个**——其中 `check-partition-plan.sh` 服务 02-3/04-2/07-5 三张卡,`verify-windows-baseline.ps1` 服务 03-1/07-4,`backup-esp.ps1` 服务 03-8/07-10,`verify-baseline.ps1`+`check-health.sh`+`check-signature.sh` 共服务 07-7;另有**库与接口 14 个**(两侧 `dbk-cli`、两侧 `dbk-obs`、`dbk-log.sh`、四个接口 `dbk-pkg.sh`/`dbk-update.sh`/`dbk-rollback.sh`/`dbk-driver.sh`、`dbk-win-probe.ps1`、`dbk.sh`、`dbk.ps1`、`verify-all.sh`、`verify-all.ps1`)、**仓库自检 4 个**(`check-docs.sh`、`check-docs-lib.sh`、`check-docs-repo.sh`、`check-scripts.sh`)与**步骤索引 2 个**(`linux/steps.tsv`、`windows/steps.tsv`)。
...
| 2026-09-25 | **设计回切(实施计划任务 1)**:第 1 节动作卡 48 -> **47 张**(首启收敛 14 -> 13,删除 snap 卡);第 4 节 C9d 白名单同步为**七个库/接口文件**(`dbk-cli`/`dbk-obs`/`dbk-log`/`dbk-pkg`/`dbk-update`/`dbk-rollback`/`dbk-driver`);第 6 节映射表按原子版口径重写(`05-3` 改薄脚本调 `dbk-driver.sh`、`05-7` 调 `dbk-update.sh`、`05-9` 改 `linux/rollback-deploy.sh` 部署级回滚、`05-10` 改 `rpm-ostree rebase`、删除 snap 卡行),合计重算为 **45 个步骤脚本 + 14 个库与接口** |
