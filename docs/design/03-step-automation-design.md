...
| 规则 | 内容 |
|---|---|
| C9a | 卡内出现的 `scripts/**/*.{sh,ps1}` 路径**必须存在** |
| C9b | 每个 `scripts/windows/*.ps1` 与 `scripts/linux/*.sh` **必须**含 `# 对应卡:` 行,且该卡号在对应文档中真实存在(白名单:库文件与仓库自检脚本) |
| C9c | **反向覆盖**:每个步骤脚本必须被至少一张卡引用(`grep` 卡内路径) |
| C9d | 白名单文件(库/自检):`scripts/linux/dbk-log.sh`、`scripts/linux/dbk-cli.sh`、`scripts/linux/dbk-pkg.sh`、`scripts/repo/check-docs.sh`、`scripts/repo/check-scripts.sh`,以及 `windows/dbk-cli.ps1` |
...
**图例**:`✓` 已存在(可能需加 `--check/--json` 契约);`新` = 本次新增;`人工` = 无法自动化(卡内标注)。
...
| 01-3 | 核对目标盘 | ✓ `windows/preflight.ps1 -Only target-disk`(需新增 `-Only`) |
...
| 03-1 | 跑只读体检 | ✓ `windows/preflight.ps1` |
| 03-2 | 读闸门结论 | 新 `windows/check-gate.ps1`(解析报告 → PASS/FAIL 并列红项) |
| 03-3 | 跑基线备份 | ✓ `windows/backup-esp.ps1`(加 `--check`:校验已有备份的 `manifest.sha256`) |
| 03-4 | 落 L2 产物 | 新 `windows/collect-l2.ps1` |
| 04-1 | UEFI 启动进 live | ✓ `windows/set-bootnext.ps1`(扩展 `-Device USB`:设置一次性从 U 盘启动) |
...
| 05-3 | 显卡驱动与 MOK | ✓ 改写 `linux/graphics.sh` + 新 `linux/graphics-mok.sh` |
| 05-4 | 时间 | 新 `linux/set-time.sh` |
| 05-5 | 蓝牙 | ✓ `linux/bt-keys-sync-wrapper.sh` |
...
| 06-2 | 备份现状 | ✓ `windows/backup-esp.ps1 -OutDir D:\dbk-l5-backup` |
...
| 07-5 | 只重装 Fedora | ✓ `linux/check-partition-plan.sh`(安装人工;含"ESP 绝不格式化"断言) |
| 07-6 | 基线回滚 | 新 `windows/restore-esp.ps1`(从 `baseline/02-esp-backup` 还原 + 逐文件校验) |
| 07-7 | 周期巡检 | ✓ `windows/verify-baseline.ps1`(加 `--json`)+ 新 `linux/check-signature.sh`(nvidia 模块签名) |
