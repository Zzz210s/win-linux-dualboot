...
现状:**47 张动作卡**(底座 4 / 分盘 4 / 轨道 W 9 / 轨道 L 4 / 首启收敛 13 / 退役与救援 13;卡号按 `01-playbook-reshape-design.md` 第 4 节的"三轨道 + 分盘前置章节"结构)对应 **18 个脚本**,其中真正覆盖动作的约 10 个,缺的恰是最危险的 L5(删分区、清 NVRAM)与 07(救援)。
...
| S2 | **L3 的 Silverblue 安装保持人工**(Anaconda 交互);用"L3 前置核对脚本(`check-partition-plan.sh`)+ L3 后置校验脚本(`verify-l3.sh`)"把这步包住。无人值守安装(kickstart)不作为 v1 路径(被否理由:出错时无人拦,分区写错即毁 Windows) |
...
**图例**:`[有]` 既有脚本(按原子版语义改写后仍在用);`新` = 本方案新增;`人工` = 无法自动化(卡内标注)。卡号取 `01-playbook-reshape-design.md` 第 4 节的"三轨道 + 分盘前置章节"结构(底座 = 01 / **分盘(安装前的前置章节)= 02** / **轨道 W = 03** / 轨道 L = 04 / 首启收敛 = 05 / 退役与救援 = 07)。

| 卡 | 动作 | 脚本 |
|---|---|---|
| 01-1 | 改固件设置 | `windows/check-firmware.ps1`(BIOS 内的开关本身只能人工) |
| 01-2 | 做两个安装介质(Windows 11 + Fedora 44 Silverblue) | `windows/verify-install-media.ps1`(校验 ISO 与官方校验值、列出可移动盘;写入由 Rufus / Ventoy 人工) |
...
| 02-3 | 轨道 L 的分盘 | `linux/check-partition-plan.sh --track L`(Anaconda 分区动作人工;**与 04-2、07-5 同一脚本**) |
...
| 04-2 | 手动分区(Anaconda) | `linux/check-partition-plan.sh --track D`(分区动作人工;含"不让 Anaconda 动 Windows ESP"的前置提示) |
| 04-3 | 装完重启验证 | `linux/verify-l3.sh`(引导包在位、GRUB 落 `\EFI\fedora\`、`/boot` 独立 ext4、两块 ESP、`BootOrder` 首位) |
| 04-4 | 落 L3 产物 | `linux/collect-l3.sh` |
| 05-1 | 共享盘挂载 | `linux/mount-shared.sh` |
| 05-2 | 家目录重定向 | `linux/xdg-redirect.sh` |
| 05-3 | 显卡与 Secure Boot(ublue 预签名镜像 + 一次性 MOK) | `linux/graphics.sh`(薄脚本,调 `dbk-driver.sh`) |
...
**合计**:动作卡 **47 张**(01 四 + 02 四 + 03 九 + 04 四 + 05 十三 + 07 十三),对应**步骤脚本 46 个**——其中 `check-partition-plan.sh` 服务 02-3/04-2/07-5 三张卡,`verify-windows-baseline.ps1` 服务 03-1/07-4,`backup-esp.ps1` 服务 03-8/07-10,`verify-baseline.ps1`+`check-health.sh`+`check-signature.sh` 共服务 07-7;另有**库与接口 11 个**(两侧 `dbk-cli`、两侧 `dbk-obs`、`dbk-log.sh`、`dbk-pkg.sh`、`dbk-win-probe.ps1`、`dbk.sh`、`dbk.ps1`、`verify-all.sh`、`verify-all.ps1`;Task 6 追加 `dbk-update.sh`/`dbk-rollback.sh`/`dbk-driver.sh` 后为 14 个)、**仓库自检 4 个**(`check-docs.sh`、`check-docs-lib.sh`、`check-docs-repo.sh`、`check-scripts.sh`)与**步骤索引 2 个**(`linux/steps.tsv`、`windows/steps.tsv`)。**口径**(本行与第 8 节代价表同口径) = 卡:各手册 `### NN-K` 标题总数(设计口径 47 张;当前手册实测 48 张,多出的 1 张是 `docs/05-first-boot.md` 里尚未删的 snap 卡(Task 12 删除该卡后两口径一致);`docs/10-faq.md` 的 `### 10-K` 是问答条目,不计入);步骤脚本:`scripts/{linux,windows}/steps.tsv` 的脚本条目去重数 = 46(当前含待删的 `step-snap-free.sh`、`rollback-pkg.sh`);库与接口 = 不带 `# 对应卡:` 的库文件数 = 11。**最终值由 Task 17 按仓库实际重算并同步**。
...
5. 夹具放在 `.superpowers/sdd/2026-09-25-atomic-restore-and-distro-interface/fixtures/<批次或脚本名>/`(**不入库**),用假的 `rpm-ostree`/`apt-get`/`dpkg`/`efibootmgr`/`lsblk`/`sgdisk`/`diskpart`/`bcdedit` 可执行文件或同名桩函数注入(本条指步骤脚本夹具;check-docs 自检脚本自身的夹具是例外,已入库到 `scripts/repo/tests/check-docs/`,见第 4 节)。
...
| 项 | 代价 |
|---|---|
| 规模 | 动作卡 **47 张** + 新增 **46 个步骤脚本** + **11 个库与总控入口**(与第 6 节合计同口径:卡 = 各手册 `### NN-K` 标题总数,设计口径 47 张、当前手册实测 48 张(多 1 张为手册里待删的 snap 卡);步骤脚本 = `scripts/{linux,windows}/steps.tsv` 的脚本条目去重数 = 46;库与总控 = 不带 `# 对应卡:` 的库文件数 = 11);仓库脚本类文件总数(不含 `.gitkeep` 与测试夹具)从 **16 个**增到 **63 个**(46 步骤脚本 + 11 库与总控 + 4 仓库自检 + 2 步骤索引);实施任务从 21 个增到约 29 个。**最终值由 Task 17 按仓库实际重算并同步** |
...
明确接受的取舍:**用"约一倍的工作量"换"每步可自动判定 + 危险步骤有断言"**;不接受的是"为了脚本数量而写无法验证的自动化"——因此 S2 明确放弃无人值守安装,人工步骤(固件开关、Anaconda、Windows 安装、激活动作)一律不做假自动化。
...
| 2026-09-25 | **设计回切(实施计划任务 1)**:第 1 节动作卡 48 -> **47 张**(首启收敛 14 -> 13,删除 snap 卡);第 4 节 C9d 白名单改为**既有 15 项 + 本次追加 3 个发行版薄接口 = 18 项**(**只追加、不删既有**;四个接口 = `dbk-pkg`/`dbk-update`/`dbk-rollback`/`dbk-driver`);第 6 节映射表按原子版口径重写(`05-3` 改薄脚本调 `dbk-driver.sh`、`05-7` 调 `dbk-update.sh`、`05-9` 改 `linux/rollback-deploy.sh` 部署级回滚、`05-10` 改 `rpm-ostree rebase`、删除 snap 卡行),合计按第 6 节口径重算为 **47 张动作卡 + 46 个步骤脚本 + 11 个库与接口**(最终值由 Task 17 重算) |
| 2026-09-25 | **脚本层规则 S-1/S-2 落地(实施计划任务 6)**:第 4 节新增两条自检规则——**S-1** 除四个发行版薄接口外 `scripts/linux/*.sh` 不得出现包管理器字面量(报 `S1_PKG_LEAK`;Task 6 之后 `check-scripts` 必红 = 待修清单,Task 10 清零)、**S-2** 仓库根只允许 5 个白名单文件且追踪文件名不得含空格(报 `S2_STRAY_ROOT`/`S2_SPACE_NAME`,源自 2026-09-25 夹具残留被误提交到仓库根的事故);`check-docs-lib.sh` 的 `WL` 按 C9d 追加 `dbk-update.sh`/`dbk-rollback.sh`/`dbk-driver.sh` 至 18 项,两处逐行一致 |
