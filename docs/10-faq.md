...
| 2 | 装完没网 / 键盘失灵,要不要换个发行版 | 先换内核(HWE)或换驱动版本,**不降发行版** | 第 2 条 |
...
| 13 | 能不能装到移动硬盘 / USB SSD | 可以,但属偏离分支(v1 未验证):供电、性能、引导条目三件事必须自己补齐 | 第 13 条 |
...
2. 按固定优先级处理:**换更新的内核(HWE 内核栈)-> 换驱动版本 -> 才谈发行版问题**;
3. 无线网卡缺固件时,安装对应版本的 `linux-firmware` 后重启验证;
4. **不降发行版、不换发行版**:换来的是更短的支持期与更差的新硬件兼容性,问题往往在半年后以更难看的形式回来;而 Ubuntu 26.04 LTS 的门槛(支持期、预签名 NVIDIA 包)正是本方案的选型理由。
...
**指向更详细的章节**:[04-ubuntu.md](04-ubuntu.md) 步骤 2(含该卡点的专门说明);[design/00-design.md](design/00-design.md) 3.20 与 11.1 第 7 条;[08-verification.md](08-verification.md) A5、A7。
...
**处置**:走 L5 退役流程,**顺序不可更换**——先改顺序、先备份,最后才删分区:

1. 只读取证(记录 `efibootmgr -v`、`lsblk`;这一步不写任何东西);
2. 在 Ubuntu 里把 `BootOrder` 首项改回 Windows Boot Manager:**只用固件设置界面**(可从 Ubuntu 执行 `sudo systemctl reboot --firmware-setup` 直达),**不得用 `efibootmgr -o`**;
3. 重启进 Windows,备份当前 NVRAM 与 ESP 现状到 `D:\dbk-l5-backup`(**不要覆盖 `baseline\`**);
4. 在 Windows「磁盘管理」里**只删 Linux 的分区**;
5. 清理 NVRAM 中残留的 `ubuntu` 条目;可选的最后一步是把腾出的空间扩展进相邻分区。

**依据与细节**:删掉 Linux 分区后,固件在失效条目之后会继续回落到下一个条目即 Windows——这正是 I1 的价值所在,它比"记得先修引导再删分区"可靠,后者依赖人的记忆。退役是**可逆的**:做到第 2 步停下,就是"暂时停用 Linux、不删分区"的变体。
...
- **想拿第二块盘装 Ubuntu 系统**:那属于双盘设备分支(设计文档第 10 节列为待编写),不在 v1 路径内。
...
**快速判断**:**技术上可以,但这是偏离分支,v1 未验证**——设计文档第 10 节把它列为待评估,尚未给出步骤。走这条路的话,下面几件事必须自己补齐并登记为偏离:
...
4. **ESP 布局要另做设计**:是"移动盘自带独立 ESP"而不是复用内置盘 ESP,因此仍受"引导盘必须 GPT + UEFI"约束(第 12 条);
...
**快速判断**:**不一定,而且第一步不是重装,而是判层**:引导层 / 系统分区 / 分区表。
...
1. **现象能被对上**:能在速查表里找到条目,且该条给出的判据是可观测的(例如 `cat /proc/cmdline` 不含 `nomodeset`、`timedatectl` 的 `RTC in local TZ: no`、[verify-baseline.ps1](../scripts/windows/verify-baseline.ps1) 的三项引导判据通过);
...
1. **速查表里找不到对应现象**:不要猜着动手。先按 [07-rescue.md](07-rescue.md) 步骤 0 判层(引导层 / 系统分区 / 分区表),再决定动作;
