...
目标状态、四条不变量(I1-I4)与参数名在[入口文档](00-overview.md)中定义;依据见[设计文档](design/00-design.md) 第 2 节(I1-I4)、4.6 节(L5 退役五步,**顺序不可更换**)、4.8 节(崩溃后原地重装两法与"第三选择")、第 7 节(故障矩阵、7.1 周期巡检与 SBAT 事故、7.2 回滚三粒度)与第 9 节;卡格式见[设计文档](design/01-playbook-reshape-design.md) 第 3 节(R1-R7)。

**先判断层级,再动手。** "进不去系统"有三种完全不同的原因——引导层损坏、系统分区损坏、硬件故障;**只有第二、三种才需要动分区**,引导层损坏一律先修引导(`07-2` / `07-3` / `07-6`),不要重装(设计 4.8 的"第三选择")。**明确禁止"先格式化 Linux 分区再修引导"**:删掉分区的一瞬间真正消失的是 `/boot` 上的 `grub2/` 与 `loader/entries/`,而固件里的 `fedora` 条目仍指着已删除的 `\EFI\fedora\grubx64.efi` 并可能排在启动顺序前面,开机就停在 `grub rescue>`——这正是该事故形态的成因,也是"必须先把引导归位、再删分区"的全部理由(设计 4.6)。

## 开始前

- 什么时候用:出事时才读(进不去、引导异常、要撤除 Linux);**先跑 `07-1` 判层,再按结论选卡**,不要跳过判层直接重装。
- 需要的东西:`baseline/02-esp-backup/`(含 `manifest.sha256`)、`02-firmware-entries.txt`、`02-partitions.txt` 在位可读(I4);常备 Fedora 安装 U 盘;BitLocker 48 位恢复密钥在手。
...
做:跑只读判层脚本采集现场,按结论选卡;本卡不改任何东西。
  1. `scripts/linux/triage.sh --check`(需 root 读 `efibootmgr -v`;读不到会报"需人工"并给 sudo 指引)
     看到:输出含 `判层结论: <引导层|系统层|ESP 层|硬件层>;建议卡号: …`,并逐条列出证据(分区/挂载/固件条目/两块 ESP 内容/部署)
...
坑:现代 GPT 盘的分区写作 `(hdX,gptY)`,照抄旧教程的 `msdosY` 会报 `unknown filesystem`;`set prefix` 指到 ESP 的 `/EFI/fedora` 里会 `insmod normal` 报 `file not found`(那里只有 stub,没有模块目录);这是**临时**修复,重启照旧。
...
坑:`bcdboot` 会重写 `bootmgfw.efi` 与 `BCD`(这是它的职责,属预期),但**绝不允许** `{bootmgr}` 的 path 或 `BootOrder` 首位变化(I1/I3);`-WindowsDir` 盘符搞错会往错的卷上写引导文件;`\EFI\fedora\` 一律不动。
...
做:只在 Windows 系统分区已损坏时用;官方 ISO 引导 -> 自定义安装 -> **逐分区核对后只格式化 `C:`(200GiB NTFS)**。
  1. 能进系统就先按 `07-10` 备份现状,并确认 BitLocker 恢复密钥在手
     看到:备份树含 `EFI\Microsoft\` 与 `EFI\fedora\` 两棵子树,`manifest.sha256` 就位
  2. 安装界面里逐项核对:`C:` 200GiB NTFS、`D:` 约 635GiB NTFS、Fedora 三块(1GiB FAT32 / 1GiB ext4 / 约 113GiB btrfs)、ESP 2GiB、MSR 16MiB、WinRE 约 1GiB
     看到:安装器只对 `C:` 执行格式化;`D:`、Fedora 各分区、ESP、MSR、WinRE 未被标成格式化,也没有"删除所有分区"
  3. 装完让安装器在 ESP 上重建 `\EFI\Microsoft\` 与 BCD,再按 `03-2`(关快速启动)、`03-3`(已知文件夹重定向到 `D:`)、`03-4`(激活)收尾
     看到:能正常进 Windows;六个已知文件夹的值以 `D:\` 开头;装完先挂 ESP 看 `\EFI\fedora\` 是否还在
脚本:scripts/windows/verify-windows-baseline.ps1(装完核对版本/分区布局/WinRE 偏差;安装动作人工)
坑:格式化不可逆,`D:` 的文档数据、Fedora root 上的代码与密钥、`/boot` 上的旧部署都只能靠外部备份;**`\EFI\fedora\` 不在 L2 基线里**(基线生成于装 Fedora 之前);安装器新建或挪动 WinRE 属已登记偏差,写进 `baseline/` 即可,不要为此挪分区。
出错时:看不到驱动器 -> 回 [01-firmware.md](01-firmware.md) 查控制器模式(AHCI / NVMe、VMD 关闭),不要反复重试;`\EFI\fedora\` 被安装器清掉 -> 按文末「重建的两条来源」重建并补条目。

### 07-5 办法二:只重装 Silverblue(只格式化 root,ESP 绝不勾格式化)

做:只在 root 损坏时用;Fedora U 盘引导 -> 手动分区 -> **只格式化 root 并挂 `/`**。
  1. 先从 live 环境抢救数据并确认现场:把 `~`(即 `/var/home`)下要留的东西 `rsync -a` 到 `/mnt/shared/` 或外置盘
     看到:数据已拷出;Windows `C:`、`D:` 都在、分区数与 `baseline/02-partitions.txt` 一致(这是"root 损坏"而不是"分区表故障")
  2. 装前跑核对脚本,再在 Anaconda 里只指定挂载点:`/boot/efi`(ESP-Fedora,复用)、`/boot`(复用)、`/`(root,唯一勾格式化)
     看到:分区列表里只有 root 那行带"格式化";ESP-Fedora 与 `/boot` 的"格式化"**未勾选**;Windows 各分区不参与挂载
  3. 装完按 `04-3` 复核,再按 `05-1` 起的各卡重放配置
     看到:`/boot/efi/EFI` 下 `fedora` 与 `Microsoft` 并存;`BootOrder` 首位仍是 Windows Boot Manager
脚本:scripts/linux/check-partition-plan.sh --track D --check(安装动作人工)
坑:**勾了"格式化 ESP"会同时清空 `\EFI\Microsoft\`,让 Windows 也进不去——这是全流程最危险的一步**(I3、设计 4.8 办法二);`/boot` 勾格式化会丢掉全部旧部署与回滚点,能否保留 `/var` 子卷**取决于安装器行为(待核实,以官方文档为准)**;点"下一步"之前退回取消是无损的。
出错时:只看到 U 盘 -> 回 [01-firmware.md](01-firmware.md) 查控制器模式;已误格 ESP -> 按 `07-6` 用基线放回 `\EFI\Microsoft\`、按 `07-3` 重建,`\EFI\fedora\` 按文末「重建的两条来源」重建。
...
做:引导层损坏而系统分区完好时,用 L2 基线把 `\EFI\Microsoft\` 放回去再重建引导;这不是重装。
  ...
  2. `-Apply -Yes`:只复制清单里 `EFI/Microsoft/` 的文件,复制后复读比对
     看到:脚本报 PASS;`\EFI\fedora\` 执行前后文件清单完全一致(本卡绝不还原、不创建它);比清单多出的 `BCD.LOG*` 记为"预期新增"
...
出错时:`\EFI\fedora\` 也没了 -> 它不在 L2 基线里,按文末「重建的两条来源」处理;`mountvol` 挂不上 ESP -> 换盘符,仍失败说明分区表层有问题,回 `07-1` 重新判层。

### 07-7 周期巡检(Windows 大版本 / 累积更新之后)

做:每次更新之后复核四项基线,顺带看 Linux 侧模块签名;只看不改。
  ...
  2. 在 Fedora 里跑签名核对与部署状态
     看到:`check-signature.sh` 报 PASS(打印签名者与已注册密钥条数)或"需人工";部署列表与 pin 状态与预期一致
  3. 把巡检输出连同结论记进 `baseline/` 或清单备注
     看到:四项结论与偏差都有文字;没有为了"让基线成立"去改 `baseline/` 的既有记录
脚本:scripts/windows/verify-baseline.ps1 -BaselineDir baseline;scripts/linux/check-signature.sh --check;scripts/linux/dbk-rollback.sh --check
坑:更新之后 ESP 出现差异要当"引导被接管"处理,先按 `07-1` 判层,而不是先重做基线;`bcdboot` 重建过 BCD 的设备,②项的差异属预期(记备注);未签名/未注册就关 Secure Boot 会破坏预签名 NVIDIA 包路径,补救走 `05-3`。
...
做:把四条纪律当硬约束;本卡无脚本。
  1. **不要反复长按电源强制重启**:优先切 TTY(`Ctrl + Alt + F3`),内核无响应时用 SysRq 安全重启(依次 `R` `E` `I` `S` `U` `B`;Fedora 默认 `kernel.sysrq=176` 只开放 `S`/`U`/`B` 时直接按 `S` -> `U` -> `B`);已强断过就先 `fsck`(设备名按 `baseline/02-partitions.txt`,如 `sudo fsck -f /dev/nvme0n1p7`)
...
做:**先归位、再删分区**(退役五步顺序不可更换);永久顺序只能在固件设置界面改,脚本只用一次性 `bootsequence`。
  1. 只读记录现状:`sudo efibootmgr -v`,抄下 `BootOrder:` 整行、`Windows Boot Manager` 编号、`fedora` 条目的编号与 loader 路径(通常 shim 与 grub 各一条)
     看到:三样都记进备注;`fedora` 条目路径形如 `\EFI\fedora\shimx64.efi` 或 `\EFI\fedora\grubx64.efi`
  ...
  4. 偏差分支(固件只给"删除条目"、不给顺序调整):① 先按 `07-10` 做备份 -> ② 回 Fedora 执行 `sudo efibootmgr -b <fedora 编号> -B` 删条目让固件回落 -> ③ 重启进 Windows 继续 `07-11`
...
做:**引导已归位、备份已做完**之后才动手;三块 Fedora 分区一并删除(ESP-Fedora 与 `/boot` 也在其中,别只删 root),Windows 侧一个都不动。
  1. 先 `-Check`,按 `baseline/02-partitions.txt` 与 `D:\dbk-l5-backup\02-partitions.txt` 的偏移/大小逐项对账确认目标(ESP-Fedora 1024MB、`/boot` 1024MB ext4、root 约 113GiB btrfs;分区号以实测为准)
...
坑:删分区不可逆,Fedora root 上的代码与密钥、`/boot` 上的旧部署全部消失;**Windows ESP 与 Windows 各分区绝不允许成为目标**;绝不用 `diskpart clean` / "删除所有分区";**绝不先格式化 Linux 分区再修引导**——那正是 `grub rescue>` 的成因(固件条目仍指着已删除的引导文件且排在前面)。
...
做:先删指向已不存在文件的 `fedora` 条目,再把腾出的空间扩给 `D:`(不是 `C:`);两件事各自有前置断言。
  1. 先 `-Check`:列出将删除的条目,并断言首位
     看到:列出 path 指向 `\EFI\fedora\` 或 description 含 fedora/silverblue 的条目;`BootOrder` 首位仍是 Windows Boot Manager;此时零写
  2. `-Apply -Yes` 删除条目(执行前后都断言首位)
     看到:目标条目消失、非目标条目仍在、`BootOrder` 逐字未变且首位仍是 Windows Boot Manager、`{bootmgr}` path 未变
...
做:做到 `07-9` 就可以停;要更干净一点就再停用条目——**不删分区、不删条目**。
  ...
  2. `-Apply -Yes`:设一次性 `bootsequence` 回落 Windows,并提示人工在固件设置界面把 `fedora` 条目移到最后
...
## `\EFI\fedora\` 不在 L2 基线里:重建的两条真实来源

`baseline/02-esp-backup/` 由 L2 预检产出,而 L2 生成于装 Fedora **之前**,所以里面**没有** `EFI/fedora/`——不能用它还原 Fedora 引导(基线只能复原 `\EFI\Microsoft\`,固件条目现状看 `baseline/02-firmware-entries.txt`)。

**(a) 做过 L5 退役备份的**:用 `D:\dbk-l5-backup\02-esp-backup\EFI\fedora\`(那是 `07-10` 写出的备份,含两棵子树):

```powershell
mountvol S: /s
robocopy D:\dbk-l5-backup\02-esp-backup\EFI\fedora S:\EFI\fedora /E
mountvol S: /d
```

条目也丢了时补建一条:`sudo efibootmgr -c -d /dev/nvme0n1 -p 5 -L fedora -l '\EFI\fedora\shimx64.efi'`(盘与分区号按 `baseline/02-partitions.txt` 替换),建完**立刻断言** `BootOrder` 首位仍是 `Windows Boot Manager`。

**(b) 没有现成备份的**:从 live 环境重建(必须以 UEFI 启动:先 `ls /sys/firmware/efi` 确认存在;盘与分区号按 `baseline/02-partitions.txt` 替换):

```bash
sudo mount /dev/nvme0n1p7 /mnt                  # Fedora root
sudo mount /dev/nvme0n1p5 /mnt/boot/efi         # ESP-Fedora
sudo mount /dev/nvme0n1p6 /mnt/boot             # /boot(独立 ext4)
for d in dev dev/pts proc sys run; do sudo mount --rbind /$d /mnt/$d; done
sudo chroot /mnt /bin/bash
grub2-install --efi-directory=/boot/efi --bootloader-id=fedora   # 它自建 NVRAM fedora 条目,不要再手工 efibootmgr -c
grub2-mkconfig -o /boot/grub2/grub.cfg          # 待核实(以官方文档为准)
exit
sudo umount -R /mnt
sudo efibootmgr -v
```

若 `grub2-install` 未写入 NVRAM(或 `efibootmgr -v` 里看不到 `fedora`),用上面的 `efibootmgr -c` 兜底补建一条,再断言首位是 `Windows Boot Manager`;出现重复条目用 `sudo efibootmgr -b <编号> -B` 清理。`shimx64.efi` 与 `grubx64.efi` 是一对,不要从别的机器或别的系统抄文件;全程保持 Secure Boot 开启(走 shim 签名链,设计 3.3)。
