...
红项共五项,逐条来自[设计文档](design/00-design.md) 4.3 与四条不变量:

1. 存储控制器命中 VMD / RAID(Linux 侧看不到磁盘);
2. BitLocker 保护已开启(改分区表会索要恢复密钥);
3. Fast Startup 已开启(`HiberbootEnabled = 1`,NTFS 双写风险);
4. 磁盘末尾未分配空间不足 115GiB(root 100 + 快照 15);
5. **I4 基线产物缺任一**(改分区表之前必须先有可用基线)。

第 5 项是"产物齐备性"而非"设备风险":初检时它通常是红的,跑完基线备份再重跑一次即转绿——这正是本阶段两步一收尾的用意。
...
- **权限与环境**:Windows 11 专业版自带的 Windows PowerShell 5.1(桌面快捷方式右键"以管理员身份运行")。非管理员时脚本仍能跑,但存储控制器、BitLocker、固件启动项、分区表等项会读不到并判黄——**闸门报告必须在管理员会话里生成**,否则结论不可用。
...
脚本只读:它不改动任何系统设置,唯一的写动作是生成报告文件。它会检查 14 项——管理员权限、存储控制器模式、Secure Boot、BitLocker 保护状态、Fast Startup、休眠文件、磁盘 0 末尾未分配空间、ESP 大小与剩余、固件启动项、L0 启动顺序基准、L1 产物复核、L1 隔离结论转记、I4 基线产物齐备、系统版本。判定规则与本文"验证"一节的表逐条一致。
...
| BitLocker 保护已开启 | ①用 `manage-bde -protectors -get C:` 取得 48 位恢复密钥并**存到设备之外**(不写进本仓库、不写进 `baseline/`);②`manage-bde -protectors -disable C: -rc 0` 挂起保护;③重跑 `preflight.ps1`,确认该行转绿。挂起是有时限的:它的价值窗口是 L2→L3,期间不要做无关的关机搁置 |
...
| 未分配空间不足 115GiB | 回 L1 处理:整盘 `clean` 后按 [templates/partitions.txt](../templates/partitions.txt) 重排重装(设计文档 3.5、5.1)。**不做任何事后缩容**,也不得削减 ESP 来凑数 |
| I4 基线产物缺失 | 不是设备问题:执行步骤 3 生成基线,再执行步骤 4 重跑脚本 |

黄项按设计文档 4.3 的闸门规则**记录后继续**,但要在报告里能看懂:管理员权限为否(须用管理员会话重跑)、BitLocker 读不到、ESP 小于 1GiB(目标 2GiB,属偏差)、`bcdedit /enum firmware` 读不到、L0 启动顺序字段缺失、L1 产物缺失、L1 隔离结论未转记、`hiberfil.sys` 仍存在。
...
2. 确认挂载点是 ESP(存在 `EFI` 目录),再用 `robocopy` 把 ESP **全量文件树**复制到 `<OutDir>\02-esp-backup\`;
3. 对复制结果逐文件算 SHA256,写成 `<OutDir>\02-esp-backup\manifest.sha256`(每行 `<哈希>  <相对路径>`,`sha256sum` 风格,路径相对 ESP 根);
...
| 核验对象 | 做法 | 不一致时 |
|---|---|---|
| 分区表未被后续操作改变 | 把报告里实测的"磁盘 0 末尾未分配空间"与"ESP 大小"对照 `baseline/01-partitions.txt` 的定稿值(设计文档 4.3 基线复核) | 写进报告"补充说明",按黄项处理;偏离到会影响 L3(未分配 < 115GiB 或 ESP 被削减)则按红项回 L1 |
...
| 8 | 未分配空间 ≥ 115GiB | 报告"磁盘 0 末尾未分配空间"行 | 数值 ≥ 115 GiB |
...
| 未分配空间不足 115GiB | 回 L1 整盘重排(见步骤 2 该行)。**不要**用缩容或削减 ESP 来腾空间 |
...
L3 完成、系统可正常启动后,在 Windows 里执行 `manage-bde -protectors -enable C:`,并用 `manage-bde -status` 确认保护已开启。挂起状态会随时间/重启自然恢复,恢复前不要做与分区表有关的事。
