...
**产物行格式(逐字,供 L2 预检提取)**:`baseline/00-firmware.md` 里这一行写成"字段名 | 实测值"两列(2-4 列都认,脚本取最后一个单元格)。逐字示例:

```text
| 启动顺序(`BootOrder` 首位)原值 | Windows Boot Manager |
```

首位是前次部署残留的 `ubuntu` 条目时,同样照实写:

```text
| 启动顺序(`BootOrder` 首位)原值 | ubuntu |
```

字段名逐字为 `启动顺序(`BootOrder` 首位)原值`。注意:本文的目标状态表、上表里带"照实记录……后续阶段比对基准"说明的那行,都是手册文本而不是产物行——照抄进 `baseline/00-firmware.md` 会被 L2 预检按假值拒收(仍判黄),必须填实测值。
