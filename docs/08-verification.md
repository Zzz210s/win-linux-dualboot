...
- [ ] A8 可撤除性演练(参考设备必做,其他设备推荐) -> 看到:另存 `\EFI\fedora\` 后删除该子树,连续重启 3 次都自动进 Windows 且无 `grub rescue`;用副本还原(或 live chroot 重建)后 A1/A3/A4/A5 复检仍通过(需人工)

脚本:BootOrder 与两块 ESP 的内容由两脚本各自判定(A1/A5/A7);A3/A4 由 Windows 侧的基线巡检判定,在 Silverblue 侧记 `需人工`;A2/A6/A8 无脚本(人工)。
...
- [ ] B2 GPU 驱动状态正常或有 nouveau 兜底,且无签名拒绝日志 -> 看到:`lsmod` 有 `nvidia`(或明确记录"回退 nouveau 兜底"的偏差),`mokutil --list-enrolled` 含上游密钥、`modinfo -F signer nvidia` 非空,`dmesg` 无 "key was rejected" / "module verification failed"(脚本判定)
- [ ] B3 Secure Boot 保持开启且未引入自签密钥 -> 看到:`mokutil --sb-state` 输出 `SecureBoot enabled`;记录里没有 `mokutil --import`、没有关闭过 Secure Boot、没有为驱动做过 DKMS(脚本判定)
- [ ] B4 共享数据分区以 `ntfs3` 读写挂载成功且带 `nofail` -> 看到:`findmnt /mnt/shared` 的文件系统为 `ntfs3`、挂载选项含 `rw` 与 `nofail`;写测试文件后根目录无残留(脚本判定)
- [ ] B5 `rpm-ostree status` 显示 ublue 镜像来源 -> 看到:`rpm-ostree status` 里当前部署的来源形如 `ghcr.io/ublue-os/...`(ublue 变体)或与该行同义的镜像来源,与计划的 `UBLUE_IMAGE` 一致(脚本判定)
- [ ] B6 分层包清单与计划一致 -> 看到:`rpm-ostree status` 的 `LayeredPackages` 只含计划内条目(必需 `smartmontools`;做过蓝牙同步时加 `chntpw`),且分层安装后已重启使清单生效(脚本判定)
- [ ] B7 跨系统双向可见性一致 -> 看到:Windows 写 `D:\Shared\dbk-verify-win.txt` 后 Silverblue 能读到逐字相同的内容(含中文与换行);反向再测一次,两侧标记文件测完删除(需人工)
...
- [ ] B10 切换系统后蓝牙无需重新配对 -> 看到:同一台蓝牙设备在 Windows/Silverblue 三趟往返里都能直接连接,不需要重新进配对模式(需人工)
- [ ] B11 `fwupd` 能识别设备 -> 看到:`fwupdmgr get-devices` 至少列出一项本机固件设备(如 UEFI 系统固件、NVMe SSD)及当前版本(脚本判定)

脚本:Silverblue 侧由 `scripts/linux/verify-all.sh` 判定 B1-B6、B8、B9、B11(B2 复用 `check-signature.sh --check`、B6 复读 `rpm-ostree status`),Windows 侧对 B1-B6、B8-B11 记 `需人工`;B7、B10 两侧都记 `需人工`。

这组全绿才可以进下一步

### C. 双系统切换组(C1-C3)

- [ ] C1 从 Windows 用一次性 BootNext(或厂商菜单键)进 Linux -> 看到:重启进入 Silverblue;`scripts/windows/set-bootnext.ps1` 退出码为 0(它自带"执行后 `BootOrder` 首位仍是 Windows Boot Manager"的断言)(需人工)
...
- [ ] C3 切换 3 次后 A 组首项检查仍成立 -> 看到:完成 3 轮 Windows -> Silverblue -> Windows 之后重跑 A1/A3/A4(必要时加 A5)全部通过,3 轮里没有出现 `grub>` / `grub rescue>`(需人工)
...
- [ ] F1 部署回滚演练(参考设备必做) -> 看到:**真做一次**:`rpm-ostree rollback` -> 重启 -> 桌面可用 -> 复检模块加载与 Wayland -> 再回滚回来;并确认"**用户数据在回滚后仍存在**"(`/var/home` 的文件在回滚前后哈希不变)(需人工)
- [ ] F2 部署固定可用 -> 看到:`scripts/linux/dbk-rollback.sh --list` 能读出 pin 状态(`已固定部署数`);变更前能 `rpm-ostree pin` 把当前部署标为 `Pinned: yes`(脚本判定)
- [ ] F3 多部署可回退 -> 看到:`rpm-ostree status` 列出至少两个部署(含版本与时间戳),GRUB 菜单能从旧部署启动(脚本判定)
- [ ] F4 崩溃可观测 -> 看到:`/var/log/journal` 存在;`journalctl --list-boots` 至少列出两条;回滚后 `journalctl -b -1` 仍能读到上一次启动的日志行(`/var` 不随部署回退)(脚本判定)
- [ ] F5 更新策略 -> 看到:`/etc/rpm-ostreed.conf` 的 `AutomaticUpdatePolicy` 只 `check`/`download`,**没有** `stage`;未启用自动应用与自动重启(脚本判定;复用 `set-updates.sh --check`)
...
脚本:Silverblue 侧由 `scripts/linux/verify-all.sh` 判定 F2-F9(F5 复用 `set-updates.sh --check`,F2/F3 复用 `dbk-rollback.sh --list`);F1 是"真做一次"的演练,两侧都记 `需人工`。
