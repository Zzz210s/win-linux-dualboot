...
目标状态、四条不变量与参数名在[入口文档](00-overview.md)中定义;分区数值与分盘动作在 `02-partitioning.md`(本文件不复述);依据见[设计文档](design/00-design.md) 4.4 节(L3)、5.1 节(分区表)与第 7 节(故障矩阵)、[变体设计](design/02-fedora-atomic-variant-design.md) 5、6 节(独立 ESP 与 `/boot`;Anaconda 已知失败)。
...
- 需要的东西:Fedora 44 Silverblue 官方安装 U 盘(ISO 已在 L0 按官方 `CHECKSUM` 校验)、参数表 `DISK_MODEL` / `DISK_SIZE`、厂商 `BOOT_MENU_KEY`。
...
做:从 Windows 侧把"下次启动"设成一次性从安装 U 盘启动,再重启进 live;不动永久启动顺序。
  ...
  2. 重启,在厂商启动菜单里选带 `UEFI:` 前缀的 Silverblue 条目
...
出错时:看不到 U 盘条目 -> 回 `01-2` 核对介质与写入方式;`\EFI\Microsoft\` 或 `BootOrder` 有异常 -> 停下按 `07-*` 处置,不要重装。

### 04-2 手动分区:三块建在 115GiB 预留区内,Anaconda 只指定挂载点

做:在 live 里手工建 Fedora 三块(ESP-Fedora 1024MB FAT32 / `/boot` 1024MB ext4 / root 约 113GiB btrfs),三块都落在 `02-partitioning` 预留的 115GiB 未分配区内;Anaconda 里**只指定挂载点**,不新建、不删除、不改尺寸。
  1. 先用核对脚本读现状,按输出的"下一步该建什么"建这三块
     看到:脚本报 PASS 且列出待建项;它同时断言 Windows ESP 未被挂载、尺寸仍是 2048MB(读不到就提示停手)
  2. Anaconda 手动分区页里只把三块指定挂载点:`/boot/efi`、`/boot`、`/`;Windows 各分区一律不挂载、不格式化、不改尺寸
     看到:分区列表新增三行且 Windows 各分区原值不变;点"下一步"前没有任何 Windows 分区被标成"格式化"
脚本:scripts/linux/check-partition-plan.sh --track D --check
坑:Windows ESP 勾了格式化 = 当场毁掉 Windows 引导(不变量 I3);Fedora ESP 或 `/boot` 勾了格式化 = Fedora 引导丢失;把 Fedora 三块建在预留区之外会挤压 Windows 分区。
出错时:分区表对不上 -> 不要就地重排,按 `07-*` 的救援流程处置;Anaconda 在写 EFI 引导配置前中止(上游已知失败 `fedora-silverblue/issue-tracker#284`,设计 6 节)-> 按 `07-*` 在 live 环境手工修,最坏退回轨道 W(`03-windows`)。

### 04-3 装完重启验证

做:装完重启,默认应仍进 Windows;进 Fedora 后用校验脚本逐项核对,而不是只看"能不能进桌面"。
  1. 进 Fedora(厂商菜单键一次性选 fedora 条目),跑校验脚本
     看到:脚本报 PASS,逐条列出 ostree 部署在位、`/boot` 独立且为 ext4、两块 ESP 内容齐全、GRUB 部署条目在位、`BootOrder` 首位仍是 Windows Boot Manager
  2. 不做任何引导改动直接重启一次,看默认进哪个系统
     看到:默认进 Windows(预期结果,不是失败);fedora 条目在 `BootOrder` 尾部
脚本:scripts/linux/verify-l3.sh --check
坑:把 fedora 条目设成默认首位会破坏 I1;`/boot` 没独立挂载时部署切换与回滚会失效(设计 3.6);在 L3 就配驱动/共享盘会把两件事混在一起(那是 `05-first-boot`)。
出错时:任一 FAIL -> 按 checks 里的失败项处置,引导层问题走 `07-*`(不重装);`BootOrder` 首位被改 -> 只在固件设置界面改回,不得用 `efibootmgr -o`。

### 04-4 落 L3 产物

做:在 Fedora 里采集六节内容落成 `baseline/03-efi-layout.txt`;先 `--check` 预览,确认后再 `--apply` 写文件。
  1. `--check` 预览六节:`\EFI\` 两棵树(Windows ESP 与 Fedora ESP)、`efibootmgr -v`、`BootOrder`、`lsblk`、`findmnt`、`rpm-ostree status` 摘要
...
坑:把产物写进仓库跟踪范围(或写进 ESP)会污染基线;只写四节而漏掉 `findmnt` / `rpm-ostree` 会让 L4 的复检缺证据。
