...
.DESCRIPTION
  只读:不修改系统任何设置,唯一的写动作是生成报告文件。本文件必须保存为 UTF-8 with BOM,否则 Windows PowerShell 5.1 会按 ANSI 解码中文而解析失败。
...
else { Add-Row '存储控制器模式' ($ctrlNames -join '; ') $GREEN }

# 3. Secure Boot
$sb = Read-RegValue 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot\State' 'UEFISecureBootEnabled'
if ($null -eq $sb) { Add-Row 'Secure Boot' '读取不到(注册表键缺失或非 UEFI 启动)' $YELLOW }
elseif ([int]$sb -eq 1) { Add-Row 'Secure Boot' 'UEFISecureBootEnabled = 1(已开启)' $GREEN }
else { Add-Row 'Secure Boot' ('UEFISecureBootEnabled = ' + $sb + '(未开启)') $YELLOW }

# 4. BitLocker 保护状态
$blProt = ''; $blVol = ''; $blDetail = ''
try {
  $blv = Get-BitLockerVolume -MountPoint $env:SystemDrive -ErrorAction Stop
  $blProt = [string]$blv.ProtectionStatus; $blVol = [string]$blv.VolumeStatus
  $blDetail = 'ProtectionStatus=' + $blProt + '; VolumeStatus=' + $blVol
} catch {
  $raw = ''
  try { $raw = (& manage-bde -status $env:SystemDrive 2>&1 | Out-String) } catch { $raw = '' }
...
if ($null -eq $hb) { Add-Row 'Fast Startup(HiberbootEnabled)' '读取不到' $YELLOW }
elseif ([int]$hb -eq 1) { Add-Row 'Fast Startup(HiberbootEnabled)' 'HiberbootEnabled = 1(快速启动已开启)' $RED }
else { Add-Row 'Fast Startup(HiberbootEnabled)' ('HiberbootEnabled = ' + $hb + '(已关闭)') $GREEN }
...
if ($esp.Count -ge 1) {
  $espGiB = [math]::Round(([int64]$esp[0].Size / 1GB), 2)
  $vol = $null
...
if ($fwOk) { Add-Row '固件启动项(bcdedit /enum firmware)' ('已读取;含 { } 标识符的行数 = ' + @(($fwText -split "`r?`n") | Where-Object { $_ -match '\{' }).Count) $GREEN }
else { Add-Row '固件启动项(bcdedit /enum firmware)' '读不到(需管理员权限或非 UEFI 启动)' $YELLOW }
...
$actText = ''
if (Test-Path -LiteralPath $a01) { $actText = Get-Content -LiteralPath $a01 -Raw -Encoding UTF8 }
...
if (-not (Test-Path -LiteralPath $p01)) { Add-Row 'L1 隔离核对结论' '01-partitions.txt 不存在,无法转记' $YELLOW }
elseif ($isoLines.Count -eq 0) { Add-Row 'L1 隔离核对结论' 'L1 注记段缺少已知文件夹重定向与 C: 内容核对结论' $YELLOW }
else { Add-Row 'L1 隔离核对结论' ('已从 01-partitions.txt 转记 ' + $isoLines.Count + ' 行到本报告') $GREEN }
...
if ($miss.Count -gt 0) { Add-Row 'I4 基线产物齐备' ('缺失:' + ($miss -join '、') + ';先跑 backup-esp.ps1 再重跑本脚本') $RED }
...
$yellows = @($rows | Where-Object { $_.Verdict -eq $YELLOW })
...
$verdictLine = '结论: 允许进入 L3'
if ($reds.Count -gt 0) { $verdictLine = '结论: 禁止进入 L3' }
