...
<#
.SYNOPSIS
  L0:校验两个安装介质(只读):ISO 是否存在、Fedora Silverblue ISO 的 SHA256 是否等于官方 CHECKSUM 值、列出可移动盘。
.DESCRIPTION
  校验值来源(设计 4.1 与 5.3):
    ① Fedora Silverblue ISO:官方发布页的 CHECKSUM 文件(路径形如
       .../releases/<版本>/Silverblue/x86_64/iso/Fedora-Silverblue-<版本>-<构建>-CHECKSUM)里的
       `SHA256 (<文件名>) = <64 位十六进制>` 行。把该文件放在与 ISO 同一目录(缺省名 <ISO 文件名>.CHECKSUM,可用 -FedoraChecksum 指定),
       本脚本逐字符比对;该文件另有官方 GPG 签名(.CHECKSUM 旁的 .asc / .gpg),签名核验由人工 gpg --verify 完成。
  ...
  退出码:0 通过 / 1 失败(ISO 缺失或 Fedora 哈希与官方值不一致) / 2 需人工(缺 CHECKSUM 文件、未声明 -WindowsOfficial) / 64 用法错误
#>
[CmdletBinding()]
param(
  [switch]$Check, [switch]$Apply, [switch]$Json, [switch]$Yes,
  [string]$Step = '', [string]$Log = '', [string[]]$Extra = @(),
  [string]$IsoDir = '.', [string]$FedoraIso = '', [string]$WindowsIso = '',
  [string]$FedoraChecksum = '', [switch]$WindowsOfficial
)
...
$fIso = Find-Iso -Given $FedoraIso -Pattern 'Silverblue'
$wIso = Find-Iso -Given $WindowsIso -Pattern 'Win11|win11|Windows|windows'

# ① Fedora Silverblue ISO:存在 + 与官方 CHECKSUM 逐字符比对
if (-not $fIso -or -not (Test-Path -LiteralPath $fIso)) {
  $failN++; $failItems += 'Fedora Silverblue ISO 缺失'
  Add-DbkCheck ('失败项:未找到 Fedora Silverblue ISO(在 ' + ([System.IO.Path]::GetFullPath($IsoDir)) + ' 下按文件名含 Silverblue 查找);用 -FedoraIso <路径> 指定')
} else {
  $fHash = Get-Sha256 -Path $fIso
  Add-DbkCheck ('Fedora Silverblue ISO 实测 SHA256:' + $fHash + '(' + (Split-Path -Leaf $fIso) + ')')
  $exp = Get-ExpectedSha256 -Iso $fIso -ChecksumFile $FedoraChecksum
  if (-not $exp) {
    $manualN++
    Add-DbkCheck ('需人工:找不到可用的官方 CHECKSUM 值(缺 ' + $fIso + '.CHECKSUM 或里面没有该文件名的 SHA256 行);从官方发布页下载 CHECKSUM 后重跑')
  } elseif ($exp -eq $fHash) {
    Add-DbkCheck ('Fedora Silverblue ISO 哈希与官方 CHECKSUM 值一致(官方值:' + $exp + ')')
  } else {
    $failN++; $failItems += ('Fedora 哈希不一致(期望 ' + $exp + ' 实际 ' + $fHash + ')')
    Add-DbkCheck ('失败项:Fedora Silverblue ISO 哈希与官方值不一致;期望(官方 CHECKSUM)' + $exp + ';实际' + $fHash + ';不要使用该 ISO,重新下载或换镜像站重下')
  }
  Add-DbkAction '人工:用 gpg --verify 核对 CHECKSUM 文件的官方签名(签名文件与 CHECKSUM 同目录,官方发布页给出)'
}
...
Write-DbkExit -Status PASS -Message '两个 ISO 都在位,Fedora 哈希与官方 CHECKSUM 一致,Windows ISO 来源已人工声明,可移动盘已列出'
