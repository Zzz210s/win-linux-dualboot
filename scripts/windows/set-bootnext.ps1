...
<#
.SYNOPSIS
  设置"下次启动进 Fedora Silverblue / 进安装 U 盘"的**一次性**固件启动条目(BootNext 语义),并断言 BootOrder 未被改动。
.DESCRIPTION
  ...
  用法(仓库根目录、管理员 Windows PowerShell):
    ...
    powershell.exe -ExecutionPolicy Bypass -File scripts\windows\set-bootnext.ps1            # 真正设置(一次性进 Silverblue)
    powershell.exe -ExecutionPolicy Bypass -File scripts\windows\set-bootnext.ps1 -Device USB -WhatIf   # 卡 04-1:一次性从安装 U 盘启动
  -Match 匹配 Fedora 条目的描述正则(默认 'fedora|silverblue|grub');-Device USB 改按可移动介质特征匹配(描述含 USB/UEFI:/Removable,或 loader 路径为 \EFI\BOOT\)。
...
param(
  [switch]$WhatIf,
  [string]$Match = 'fedora|silverblue|grub',
  [string]$Guid = '',
  [string]$Device = ''
)
...
if ($Device -and $Device -ne 'USB') {
  Write-Host ('错误:-Device 只认 USB(实为 ''' + $Device + ''');一次性进 Fedora Silverblue 用默认的 -Match/-Guid,不要用 -Device。本次未执行任何命令。')
  exit 1
}
$pathMatch = ''
if ($Device -eq 'USB') {
...
if ($Device -eq 'USB') {
  ...
  Write-Host 'set-bootnext:设置一次性固件启动条目进 Fedora Silverblue(BootNext 语义;本脚本不改动 BootOrder)'
}
...
if (-not $target) {
  if (-not $fw) {
    ...
    if ($Device -eq 'USB') {
      ...
      Write-Host '兜底路径:开机按厂商启动菜单键(BOOT_MENU_KEY)一次性选 fedora;若条目确实缺失,按 docs/04-silverblue.md 出错时一节重建条目。'
    }
  }
...
Write-Host '处置(见 docs/04-silverblue.md 出错时):只在固件设置界面把 Windows Boot Manager 改回首位,并把偏差写进 L4 记录;不得用 bcdedit displayorder 或 efibootmgr -o 改序(I2)。'
exit 1
