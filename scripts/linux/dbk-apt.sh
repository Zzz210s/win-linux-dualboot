...
# 调用约定:调用方先 source dbk-log.sh(提供 log),并在调用 apt_* 之前设置 SKIP_APT(0/1;通常取自 DBK_SKIP_APT),然后使用:
...
# 注意:SKIP_APT 不在这里固化默认值(源文件时赋值会先于调用方赋值,顺序难以保证),而在函数内用 ${SKIP_APT:-0} 实时读取。
...
apt_install() {
  local out
  if [ "${SKIP_APT:-0}" = 1 ]; then log "DBK_SKIP_APT=1:跳过 apt-get install $*"; return 9; fi
