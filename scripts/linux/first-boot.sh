...
#      日志目录不可写时(非 root)自动回落到 <TMPDIR>/dbk-<uid>/ 并打印警告。
...
# 日志目录:默认 /var/log/dbk;不可写时(通常是非 root 的 dry-run)回落到临时目录,便于离线演练
if ! mkdir -p "$LOG_DIR" 2>/dev/null; then
  LOG_DIR="${TMPDIR:-/tmp}/dbk-$(id -u)"
  mkdir -p "$LOG_DIR" || { echo "错误: 无法创建日志目录 $LOG_DIR" >&2; exit 1; }
  echo "警告: /var/log/dbk 不可写(通常因为非 root),日志改落 $LOG_DIR" >&2
fi
...
run_module() {
  ...
  # 关键输出:优先取模块自己打的 DBK-RESULT/校验行,最多两行;去掉行首时间戳,便于摘要阅读
  key="$(grep -hE 'DBK-RESULT|DBK-MODULE|校验通过|校验未通过|完成:|错误|失败' "$mlog" 2>/dev/null | tail -n 2 \
...
write_summary() {
  ...
  } >"$SUMMARY" 2>/dev/null || log "警告: 无法写入摘要 $SUMMARY"
  [ -r "$SUMMARY" ] || return 0
  log "摘要 $SUMMARY:"
