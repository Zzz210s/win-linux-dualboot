...
#   系统层 = baseline/02-partitions.txt 分区行数与现场 lsblk 的 TYPE="part" 数不一致,或已进原子版系统(部署列表可读)
#     但 `journalctl -b -p err` 错误行 ≥20(建议 07-4 / 07-5);
...
# 涉及发行版差异的部分一律走接口(dbk-rollback.sh):部署列表 = 本机是否已装好原子版系统 + 是否有可用部署。
# 注入钩子(真机留空;夹具用。取值 = 命令名或可带参数的命令行(由夹具在 PATH 注入假命令),或一个存在的文件路径(回放该文件)):
#   DBK_LSBLK / DBK_BLKID / DBK_FINDMNT / DBK_EFIBOOTMGR / DBK_JOURNALCTL / DBK_SMARTCTL / DBK_SHA256SUM / DBK_UNAME
...
#   DBK_RPM_OSTREE(经 dbk-rollback.sh 读取的部署列表命令)。
# 待核实(以官方文档为准):efibootmgr -v / journalctl -b -p err / smartctl -H / findmnt 的文本解析,
#   以及部署列表的字段口径 —— 均未在真机验证。夹具级验证,真机未跑。
# 用法: triage.sh [--check|--apply] [--json] [--log <路径>] [--step NN-K]
set -euo pipefail
...
EFI_HOOK="${DBK_EFIBOOTMGR:-efibootmgr}"
...
# 2) 取证:分区 / 设备 / 挂载 / 固件条目 / 两块 ESP 内容 / 固件 / 部署列表与错误日志 / SMART
...
# 系统层证据:部署列表(经 dbk-rollback.sh;原子版语义)、本次启动的错误日志量
DEP_LIST=""; DEP_RC=0; DEP_N=0
if DEP_LIST="$(deployments_list)"; then DEP_RC=0; else DEP_RC=$?; fi
if [ "$DEP_RC" -eq 0 ]; then DEP_N="$(printf '%s\n' "$DEP_LIST" | grep -c . || true)"; fi
...
if [ "$DEP_RC" -eq 0 ]; then
  ...
  note_manual "部署列表取不到(status 无 Deployments 段或解析不了):无法核对系统层(在救援环境里属预期)"
  if [ "$ROOT_FS" = btrfs ]; then L_SYS=1; HITS+=("系统层(已进原子版系统但读不到部署列表)"); fi
fi
...
if [ "$ESP_READ" -eq 0 ]; then
  note_manual "Linux ESP 未挂载($ESP_MNT 不存在):看不到 EFI/fedora 内容(sudo mount -o ro <ESP 分区> /boot/efi 后重跑)"
...
dbk_add_check "取证:部署列表=$([ "$DEP_RC" -eq 0 ] && printf '可读(%s 个部署)' "${DEP_N:-0}" || printf '取不到');journalctl -b -p err 行数=${JRNL_N:-0}"
dbk_add_check "取证:Windows 条目=${WBM_LINE:-未取到};fedora 条目=${FED_LINE:-未取到}"
dbk_add_check "取证:Linux ESP=$ESP_MNT(可读=$ESP_READ,EFI/fedora 在位=$FED_OK);Windows ESP=${WIN_MNT:-未挂载}(可读=$WIN_READ,bootmgfw.efi 在位=$WIN_OK)"
...
elif [ "$L_SYS" -eq 1 ]; then LAYER="系统层"; CARDS="07-4(只重装 Windows)/ 07-5(只重装 Linux 侧)"
...
dbk_add_check "失败项: 探针全部可读但没有命中任何一层(引导条目、分区数、ESP 清单、部署列表与错误日志、SMART 都看不出问题)"
