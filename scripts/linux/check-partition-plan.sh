#!/usr/bin/env bash
# 对应卡:02-3,04-2,07-5
# L3 前置核对(在 Fedora live 环境里跑):读当前磁盘布局(lsblk / blkid / sgdisk -p)与定稿分区计划比对,
# 输出"下一步该建什么";并硬断言 Windows ESP 未被挂载、未被改动(读不到 → FAIL 并提示停手)。
# 定稿值(设计 5.1,一字不改):ESP-Windows 2048MiB / MSR 16MiB / C: 204800MiB / D: 650240MiB /
#   ESP-Fedora 1024MiB(FAT32,挂 /boot/efi)/ /boot 1024MiB(ext4)/ root ≈113GiB(btrfs);轨道 D 预留区 115GiB。
# 人工边界:分区与 Anaconda 的点击都是人工(设计 S2);本脚本无写动作,--apply 与 --check 输出相同。
# 轨道:--track D(缺省;双系统,Fedora 三块建在 115GiB 预留区内)/ --track L(只装 Silverblue,整盘三块)。
# 夹具级验证,真机未跑。用法:
...
if [ "$DBK_MODE" = apply ]; then dbk_note "说明: 本脚本无写动作(分区与 Anaconda 的点击都是人工);--apply 与 --check 输出相同。"; fi

WIN_ESP_MIB=2048; FED_ESP_MIB=1024; BOOT_MIB=1024; ROOT_MIN_MIB=115712; RESERVE_MIB=117760
lb_get() { printf '%s\n' "$1" | tr ' ' '\n' | sed -n "s/^$2=\"\(.*\)\"\$/\1/p" | head -n1; }
to_mib() { awk -v b="${1:-0}" 'BEGIN{printf "%d", b/1048576}'; }

if ! command -v lsblk >/dev/null 2>&1; then
  dbk_add_check "缺少 lsblk"
  dbk_exit FAIL "缺少 lsblk,无法读取磁盘布局;请在 Fedora live 环境里运行本脚本"
fi
...
for i in "${!P_NAME[@]}"; do
  ...
  case "$fs" in
    vfat)
      if [ "$m" -ge $((FED_ESP_MIB - 100)) ] && [ "$m" -le $((FED_ESP_MIB + 100)) ]; then FED_ESP="$i"
      else ISSUES+=("ESP-Fedora 尺寸错:期望 ${FED_ESP_MIB}MiB,实际 ${m}MiB(${P_NAME[$i]})"); fi ;;
    ext4)
      if [ "$m" -ge $((BOOT_MIB - 100)) ] && [ "$m" -le $((BOOT_MIB + 100)) ]; then BOOT_P="$i"
      else ISSUES+=("/boot 尺寸错:期望 ${BOOT_MIB}MiB,实际 ${m}MiB(${P_NAME[$i]})"); fi ;;
    btrfs)
      if [ "$m" -ge "$ROOT_MIN_MIB" ]; then ROOT_P="$i"
      else ISSUES+=("root 尺寸错:期望 ≥${ROOT_MIN_MIB}MiB(约 113GiB),实际 ${m}MiB(${P_NAME[$i]})"); fi ;;
  esac
done
if [ "$FED_ESP" -ge 0 ]; then dbk_add_check "ESP-Fedora: /dev/${P_NAME[$FED_ESP]} ${FED_ESP_MIB}MiB(vfat)"; fi
if [ "$BOOT_P" -ge 0 ]; then dbk_add_check "/boot: /dev/${P_NAME[$BOOT_P]} ${BOOT_MIB}MiB(ext4)"; fi
if [ "$ROOT_P" -ge 0 ]; then dbk_add_check "root: /dev/${P_NAME[$ROOT_P]}(btrfs,≥${ROOT_MIN_MIB}MiB)"; fi
...
step=1
if [ "$FED_ESP" -lt 0 ]; then dbk_note "  $step. ESP-Fedora: ${FED_ESP_MIB}MiB FAT32 -> 挂载点 /boot/efi"; step=$((step + 1)); fi
if [ "$BOOT_P" -lt 0 ]; then dbk_note "  $step. /boot: ${BOOT_MIB}MiB ext4 -> 挂载点 /boot"; step=$((step + 1)); fi
if [ "$ROOT_P" -lt 0 ]; then dbk_note "  $step. root: 约 113GiB btrfs -> 挂载点 /"; step=$((step + 1)); fi
if [ "$step" -eq 1 ]; then dbk_note "  三块已就绪:Anaconda 里只指定挂载点(/boot/efi、/boot、/),不新建、不格式化 Windows 侧"; fi
if [ "$TRACK" = D ]; then dbk_note "  三块都建在 115GiB 预留区内;Windows 各分区一律不挂载、不格式化、不改尺寸。"; fi
dbk_add_check "Fedora 三块:ESP-Fedora=$([ "$FED_ESP" -ge 0 ] && printf 就绪 || printf 待建) /boot=$([ "$BOOT_P" -ge 0 ] && printf 就绪 || printf 待建) root=$([ "$ROOT_P" -ge 0 ] && printf 就绪 || printf 待建)"
dbk_exit PASS "分区计划核对通过:Windows ESP 未被挂载/未被改动;下一步按上面的清单建 Fedora 三块(轨道 $TRACK)"
