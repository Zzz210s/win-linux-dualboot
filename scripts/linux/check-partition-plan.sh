#!/usr/bin/env bash
# 对应卡:02-3,04-2,07-5
# L3 前置核对(在 Fedora Silverblue live 环境里跑):读当前磁盘布局(lsblk / blkid / sgdisk -p)与定稿分区计划比对,
# 输出"下一步该建什么";并硬断言 Windows ESP 未被挂载、未被改动(读不到 → FAIL 并提示停手)。
# 定稿值(设计 5.1 / 设计 06 第 4 节,一字不改):ESP-Windows 2048MiB / MSR 16MiB / C: 204800MiB / D: 650240MiB /
...
# 轨道:--track D(缺省;双系统,Fedora 三块建在 115GiB 预留区内)/ --track L(只装 Fedora,整盘三块)。
# 夹具级验证,真机未跑。用法:
...
if ! command -v lsblk >/dev/null 2>&1; then
  ...
  dbk_exit FAIL "缺少 lsblk,无法读取磁盘布局;请在 Fedora Silverblue live 环境里运行本脚本"
fi
...
# Fedora 三块(只看目标盘;按文件系统类型 + 尺寸找候选,尺寸不符即 FAIL)。root 定稿为 btrfs,也兼容 ext4。
...
for i in "${!P_NAME[@]}"; do
  ...
  case "$fs" in
    ...
    btrfs)
      ...
      else ISSUES+=("btrfs root 尺寸不足:期望 ≥${ROOT_MIN_MIB}MiB(约 113GiB),实际 ${m}MiB(${P_NAME[$i]})"); fi ;;
  esac
done
...
if [ "$step" -eq 1 ]; then dbk_note "  Fedora 三块已就绪:Anaconda 里只指定挂载点(/boot/efi、/boot、/),不新建、不格式化 Windows 侧"; fi
dbk_note "  坑(设计 06 第 5 节):Anaconda 没有独立的'引导器位置'选项,它把引导装到 /boot/efi 指向的那块 ESP ——"
...
dbk_add_check "Fedora 三块:ESP-Fedora=$([ "$UBU_ESP" -ge 0 ] && printf 就绪 || printf 待建) /boot=$([ "$BOOT_P" -ge 0 ] && printf 就绪 || printf 待建) root=$([ "$ROOT_P" -ge 0 ] && printf 就绪 || printf 待建)"
